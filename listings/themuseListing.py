...
class TheMuse:
    ...
    def extract(self, url):
        MuseHelper(url=url, db=self.db, currentday=self.currentday, salaries=self.salaries, proxyOn=self.proxyOn, headers=self.headers)
        return
    ...
    def run_all(self):
        ...
        print('here2')
        MuseHelper(url=self.url, db=self.db, currentday=self.currentday, salaries=self.salaries, proxyOn=self.proxyOn, headers=self.headers).get_listing()
        return
